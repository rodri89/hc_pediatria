<form class="card background_panel_consulta_actual" id="seccion_conductas_nueva_consulta">	
	...
	<div id="seccion_conductas" hidden>		
		...
		<div class="row margin_left_5px">		
			<textarea class="form-control width650px_cel" id="conductas_detalles" name="conductas_detalles" rows="10" cols="100"></textarea>	
		</div>
...
<div id="seccion_conductas_consulta" hidden>			
</div>
...
<script type="text/javascript">
	...
	function cargarConductasConsulta(){
		var consulta = document.getElementById("consulta_id").value;
		var paciente = document.getElementById("paciente_id").value;		
		borrarSeccion("seccion_conductas_consulta");
		document.getElementById("seccion_conductas_nueva_consulta").hidden = true;
		$.ajax({
           type:'POST',
           dataType:'JSON',
           ...
           data:{consulta:consulta, paciente:paciente,_token: '{{csrf_token()}}'},
           	success:function(data){                         		
           		if(data.response_data != null && data.response == 1){           		
           			document.getElementById("seccion_conductas_consulta").hidden = false;
           			var seccion_resumen = document.getElementById("seccion_conductas_consulta");
           			addTituloDescripcion("Conductas", data.response_data.descripcion, seccion_resumen);           			          			
            	} else {
            		document.getElementById("seccion_conductas_consulta").hidden = true;
            	}
            }
        });	
	}
