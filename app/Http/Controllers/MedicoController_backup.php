...
class MedicoController extends Controller
{
    ...
    function nuevaConsultaOpciones(){
        ...
        return view('medico.nueva_consulta_opciones')->with('paciente', $paciente);    
    }
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   function existeAntecedentesPersonales($consulta_id, $paciente_id){
    ...
    function getDesarrolloMadurativoFecha(Request $request){
         $consulta_id = $request->consulta_id;                  

         $response_data = DB::table('consultas')
                        ->where('consultas.id', $consulta_id)                        
                        ->where('consultas.activo', 1)
                        ->first();

        return response()->json(array('response_data'=>$response_data));                         
    }
    ...
    function cargarDesarrolloMadurativo(Request $request){
        ...
        if($response->count() > 0) {
            ...
            if($mes != -2)
                $mes = $dm->mes;*/
            $res = 1;
            if($response->count() == 1 && $response[0]->desarrollo_madurativo_id == $observacion_id){
                $res = 0;
            }
    ...
    function nuevoInternaciones(Request $request) {
        $consulta_id = $request->consulta;
        $paciente_id = $request->paciente;
        $internaciones_id = $request->internacion_id;
        ...
        $internaciones_aux = DB::table('internaciones')                                
                                ...
                                ->get();        
        $numero = $internaciones_aux->count() + 1;               
        ...
        return response()->json(array('response'=>1, 'internaciones'=>$internaciones, 'numero'=>$numero));
    }
