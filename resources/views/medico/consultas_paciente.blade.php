      ...
      <div class="modal-body"> 
        ...
        <div class="modal-footer" style="display: flex; justify-content: space-between;">
            <button type="button" 
             ...
             style="border: 2px solid #dc3545; color: #dc3545; background: transparent;"
             onclick="desactivarConsultaResumen()">Desactivar</button>
            <button type="button" 
             class="rodri_button_aceptar"             
...
<div id="modal_resumen_consulta_paciente_consulta_prenatal" class="modal" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <div class="modal-body"> 
        <div class="modal-header">       
          <h4 class="modal-title"         
            id="modalTitleMensaje_consulta_prenatal">Resumen Consulta</h4><p id="edad_mostrar_consulta_prenatal" class="margin_right_5px cel_size_input"></p>
        </div> 
        <div id="seccion_resumen_consulta_prenatal margin_left_20px">                    
...
<div id="modal_resumen_consulta_paciente_lactancia" class="modal" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <div class="modal-body"> 
        <div class="modal-header">       
          <h4 class="modal-title"         
            id="modalTitleMensaje_lactancia">Resumen Consulta</h4><p id="edad_mostrar_lactancia" class="margin_right_5px cel_size_input"></p>
        </div> 
        <div id="seccion_resumen_lactancia margin_left_20px">                    
...
<script type="text/javascript">
    ...
    function verResumen(consulta_id, edad_paciente, tipo_consulta, edad_mostrar, fecha_consulta){
      ...
      if(tipo_consulta == 5){  // consulta prenatal
        if(edad_mostrar.localeCompare('') != 0){
          document.getElementById("edad_mostrar_consulta_prenatal").innerHTML = 'Edad: '+edad_mostrar;
        } else {
          document.getElementById("edad_mostrar_consulta_prenatal").innerHTML = 'N/A';
        }
        cargarFamiliaConsulta();  
        cargarEmbarazoActualConsulta();
        cargarAntecedentesObstetricosConsulta();      
        $('#modal_resumen_consulta_paciente_consulta_prenatal').modal('show');     

      } else {

        if(tipo_consulta == 6){  // lactancia          
          if(edad_mostrar.localeCompare('') != 0){
            document.getElementById("edad_mostrar_lactancia").innerHTML = 'Edad: '+edad_mostrar;
          } else {
            document.getElementById("edad_mostrar_lactancia").innerHTML = 'N/A';
          }
          cargarDetalleConsultaConsulta();
          $('#modal_resumen_consulta_paciente_lactancia').modal('show');     
    ...
    function desactivarConsultaResumen(){
      var consulta_id = document.getElementById("consulta_id").value;
      $.ajax({
        type: 'POST',
        dataType: 'JSON',
        url: '/desactivar_consulta',
        data: { consulta_id: consulta_id, _token: '{{ csrf_token() }}' },
        success: function(data) {
          if (data.response == 1) {
            $('#modal_resumen_consulta_paciente').modal('hide');
            if (typeof mostrarSnackbar === 'function') {
              mostrarSnackbar('Consulta desactivada');
            } else {
              alert('Consulta desactivada');
            }
            window.location.reload();
          } else {
            if (typeof mostrarSnackbar === 'function') {
              mostrarSnackbar('No se pudo desactivar la consulta');
            } else {
              alert('No se pudo desactivar la consulta');
            }
          }
        },
        error: function() {
          if (typeof mostrarSnackbar === 'function') {
            mostrarSnackbar('Error al desactivar la consulta');
          } else {
            alert('Error al desactivar la consulta');
          }
        }
      });
    }
